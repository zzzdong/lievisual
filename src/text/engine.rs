...
fn with_cached_context<R>(f: impl FnOnce(&mut FontContext, &mut LayoutContext<Color>) -> R) -> R {
...
fn push_style_diff(
    ...
    if s.font_width != base.font_width {
        builder.push(
            StyleProperty::FontWidth(parley::style::FontWidth::from_ratio(s.font_width.ratio())),
            range.clone(),
        );
    }
...
pub fn register_font(source: FontSource, family_name_override: Option<&str>) -> Result<(), String> {
